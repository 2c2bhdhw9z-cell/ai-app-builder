...
8. THE "Technical Workbench" (`technical-workbench`) Workspace_Experience SHALL present an IDE-style developer surface (file/tool panels alongside the editor, chat/compose area, and Activity_Stream) whose design is **inspired by** agentic coding assistants such as Kiro, implemented as an **original, clean-room layout** owned by this project — it SHALL NOT copy any third party's name, logo, proprietary visual design, or code, SHALL NOT imply affiliation with or endorsement by any third party (including AWS/Kiro), and SHALL remain fully theme-able via the Theme setting (Requirement 29). WHERE the platform exposes a web surface, THE AI_App_Builder SHALL display an "inspired by" acknowledgement (e.g. "Inspired by tools like Kiro") as an honest, non-affiliated credit; the acknowledgement is presentational and SHALL NOT alter any Project data or other setting.
